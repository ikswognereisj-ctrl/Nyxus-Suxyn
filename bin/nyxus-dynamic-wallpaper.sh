...
case "$set_name" in
  alien|cosmos|darkmirror|watercolor|"")
    # GALAXY only — old set names alias here so stale configs cannot
    # resurrect deleted cinematic / cream walls.
    # 2026-08-08: this map was the previous generation, and [dusk] named
    # "graffiti-space" -- a wallpaper deleted on 2026-07-30. Dusk had been
    # resolving to nothing for over a week: the wallpaper simply did not
    # change at dusk and nothing reported an error.
    #
    # Now the Suxyn set, chosen so the arc reads as a day: the horizon at
    # dawn, the galaxy sky through the day (the owner's 08-09 "one galaxy sky"
    # ruling, WIP-208 — the rose hero is retired from the default arc), the
    # liquid horizon at dusk, the
    # twin moons at night. Every name is in theme/wall-rotation.list, which
    # gate 13c checks against.
    declare -A imgs=(
      [dawn]="suxyn-horizon-sweep" [day]="suxyn-voyage-galaxy"
      [dusk]="suxyn-liquid-horizon" [night]="suxyn-twin-moons")
    ;;
  custom)
    declare -A imgs=(
