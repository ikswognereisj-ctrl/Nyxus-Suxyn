...
# 2026-08-05: two of these corners could never have fired, and a corner that
# does nothing when you shove the pointer into it is indistinguishable from a
# broken mouse. Both now call the shell that actually runs.
#   mission_control was ["nyxus-mission-control-toggle"] — a script the fork
#     stripped with the security stack. Task View IS this build's overview, and
#     it is a Quickshell surface reached over the same IPC as Super+Tab.
#   control_center was ["eww", "open", "--toggle", "quicksettings"] — eww was
#     deleted on 2026-08-03 (363 files) and is not in packages.x86_64. The
#     flyout replaced it, and is what Super+A / Super+N open.
# `qs ipc call nyxus <fn>` is the one documented way into the running shell
# (shell.qml IpcHandler); Hyprland's own keybinds use these exact calls.
ACTIONS: Dict[str, list[str]] = {
    "none":            [],
    "mission_control": ["qs", "ipc", "call", "nyxus", "taskview"],
    ...
    "control_center":  ["qs", "ipc", "call", "nyxus", "flyout"],
